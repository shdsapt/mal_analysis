...
def analyze_url_reputation(url):
            ...
            try:
                ...
                if 'NotFoundError' not in str(e):
                    ...
                    try:
                        ...
                        elapsed = 0
                        VT_SCAN_TIMEOUT = 60
                        while elapsed < VT_SCAN_TIMEOUT:
                            time.sleep(5)
                            elapsed += 5
                            print(f"    [-] 분석 대기 중... ({elapsed}/{VT_SCAN_TIMEOUT}초)")
                            analysis_resp = _requests.get(
                                f"https://www.virustotal.com/api/v3/analyses/{analysis_id}",
                                headers={"x-apikey": VT_API_KEY}, timeout=30
                            )
                            if analysis_resp.status_code == 200:
                                if analysis_resp.json().get("data", {}).get("attributes", {}).get("status", "") == "completed":
                                    try:
                                        url_info = client.get_object(f"/urls/{url_id}")
                                        _print_url_result(url_info)
                                        status_success = True
                                    except Exception as fetch_e:
                                        print(f"[!] 스캔은 완료되었으나 결과 조회에 실패했습니다: {fetch_e}")
                                    break
                        if not status_success:
                            print(f"[!] 시간 초과 혹은 오류로 스캔 결과를 얻을 수 없습니다.")
                            
                    except Exception as post_e:
                        print(f"[!] VT URL 신규 스캔 처리 실패: {post_e}")
    ...
    return status_success, is_cached
...
def analyze_urls_from_file(urls_file):
    ...
    for i, url in enumerate(urls):
        ...
        success, is_cached = analyze_url_reputation(url)
        _increment_daily_vt_count()
        ...
        try:
            with open(log_file, "a", encoding="utf-8") as lf:
                ...
                if is_cached:
                    lf.write(f"[{now_str}] [{i+1}/{len(urls)}] VT 조회 완료 (기존 이력 - 대기 생략): {url}\n")
                elif i < len(urls) - 1:
                    lf.write(f"[{now_str}] [{i+1}/{len(urls)}] VT 조회 완료 (신규 스캔 - 대기 {VT_URL_DELAY}초): {url}\n")
                else:
                    lf.write(f"[{now_str}] [{i+1}/{len(urls)}] VT 조회 완료 (마지막 URL): {url}\n")
        except Exception as e:
            print(f"[!] VT 로그 기록 실패: {e}")

        # 다음 URL 대기 (신규 스캔 시에만 16초 대기)
